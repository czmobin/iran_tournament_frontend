...
اگه میخوای پروژه رو روی سرور بالا بیاری (با بکند Django روی همون سرور):
...
- فایل `.env` رو با IP صحیح می‌سازه
- تصاویر background رو کپی می‌کنه
- پروژه رو build می‌کنه
- آماده اجرا می‌کنه

سپس با PM2 یا Screen اجراش کن (دستورات رو اسکریپت نشون میده).

### روش دستی

```bash
# 1. ایجاد .env با IP سرور
...
📖 **راهنمای کامل:** [SERVER_SETUP.md](./SERVER_SETUP.md)
